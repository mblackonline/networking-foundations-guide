...
Nothing here is tied to Cisco, Juniper, or any other vendor's equipment or certification track. The concepts apply everywhere, because the protocols are the same regardless of whose hardware or cloud is carrying them.

## How the Hands-On Parts Work

Most of this guide can be followed on the computer you already own. Every module shows commands for Windows first, with the Linux and macOS equivalents alongside them.

Some concepts only make sense when you can break them on purpose. Those exercises use a small optional lab of two virtual machines, one Windows and one Linux. The lab is entirely optional, and every module is written so that skipping it still leaves you with a complete picture. Module 2 covers the setup.
