...
- <span id="acknowledgment-ack"></span>**[Acknowledgment (ACK)](https://www.rfc-editor.org/info/rfc9293):** A TCP flag used to confirm receipt of data or another TCP message.
...
- <span id="application-layer"></span>**[Application layer](https://www.rfc-editor.org/info/rfc1122):** The layer where protocols provide services to applications, such as web access, name resolution, and email.
...
- <span id="authoritative-dns-server"></span>**[Authoritative Domain Name System server](https://www.rfc-editor.org/info/rfc1034):** A server that holds the official Domain Name System (DNS) records for a domain or zone.
...
- <span id="border-gateway-protocol-bgp"></span>**[Border Gateway Protocol (BGP)](https://www.rfc-editor.org/info/rfc4271):** The routing protocol used to exchange reachability information between large networks, including networks that form the internet.
- <span id="broadcast"></span>**[Broadcast](https://www.rfc-editor.org/info/rfc919):** Traffic sent to every device in the same broadcast domain.
- <span id="broadcast-address"></span>**[Broadcast address](https://www.rfc-editor.org/info/rfc919):** The final address in a traditional IPv4 subnet, used to reach all IPv4 hosts on that subnet.
...
- <span id="default-deny"></span>**[Default-deny](https://csrc.nist.gov/pubs/sp/800/41/r1/final):** A filtering policy that blocks traffic unless a rule explicitly permits it.
- <span id="default-gateway"></span>**[Default gateway](https://www.rfc-editor.org/info/rfc1122):** The router a host sends traffic to when the destination is outside the local subnet.
- <span id="default-route"></span>**[Default route](https://www.rfc-editor.org/info/rfc1812):** The route used when no more specific route matches a destination.
- <span id="dynamic-host-configuration-protocol-dhcp"></span>**[Dynamic Host Configuration Protocol (DHCP)](https://www.rfc-editor.org/info/rfc2131):** A protocol that automatically supplies clients with IP addressing and other network settings.
- <span id="dhcp-lease"></span>**[DHCP lease](https://www.rfc-editor.org/info/rfc2131):** A time-limited assignment of network settings from a DHCP server to a client.
- <span id="dhcp-relay"></span>**[DHCP relay](https://www.rfc-editor.org/info/rfc1542):** A device or service that forwards DHCP messages between clients and a DHCP server on another network.
- <span id="dora"></span>**[Discover, Offer, Request, Acknowledgment (DORA)](https://www.rfc-editor.org/info/rfc2131):** The four-message exchange commonly used when a client obtains a new DHCP lease for IPv4.
- <span id="domain-name"></span>**[Domain name](https://www.rfc-editor.org/info/rfc1034):** A name in the DNS hierarchy, such as `example.com`.
- <span id="domain-name-system-dns"></span>**[Domain Name System (DNS)](https://www.rfc-editor.org/info/rfc1034):** The distributed system that stores information about names, including the IP addresses associated with them.

## E

- <span id="encapsulation"></span>**[Encapsulation](https://www.rfc-editor.org/info/rfc1122):** The process of adding protocol information around data as it moves down the networking layers.
...
- <span id="ethernet"></span>**[Ethernet](https://www.rfc-editor.org/info/rfc894):** A common technology for moving frames across a local wired network.
- <span id="egress"></span>**[Egress](https://csrc.nist.gov/pubs/sp/800/41/r1/final):** Traffic leaving a system or network boundary. It is also called outbound traffic.
...
- <span id="ingress"></span>**[Ingress](https://csrc.nist.gov/pubs/sp/800/41/r1/final):** Traffic entering a system or network boundary. It is also called inbound traffic.
...
- <span id="packet"></span>**[Packet](https://www.rfc-editor.org/info/rfc1122):** The unit of data carried by IP from a source host toward a destination host.
...
- <span id="stateful-firewall"></span>**[Stateful firewall](https://csrc.nist.gov/pubs/sp/800/41/r1/final):** A firewall that remembers active network flows and can recognize their return traffic.
- <span id="stateless-filter"></span>**[Stateless filter](https://csrc.nist.gov/pubs/sp/800/41/r1/final):** A filter that evaluates packets without remembering an active connection.
